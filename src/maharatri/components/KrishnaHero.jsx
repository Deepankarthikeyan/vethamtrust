...
function Slide({ slide, isActive }) {
  return (
    <div className={`kr-slide${isActive ? ' is-active' : ''}`} aria-hidden={!isActive}>
      <div className="kr-bg" role="presentation" style={{ backgroundImage: `url(${asset(slide.bg)})` }} />
      ...
      <div className="kr-content">
        ...
        <Link to="/contact" className="kr-btn">{slide.cta}</Link>
      </div>
    </div>
  );
}
...
export default function KrishnaHero({ id, slides, compact = false, enableKeyboard = false }) {
  ...
  return (
    ...
    >
      {slides.map((slide, i) => (
        <Slide key={slide.subtitle} slide={slide} isActive={i === current} />
      ))}
